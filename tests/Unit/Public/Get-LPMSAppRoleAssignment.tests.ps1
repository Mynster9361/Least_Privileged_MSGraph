BeforeAll {
    ...
    if ($moduleInfo) {
        ...
        $publicFunction = Get-ChildItem -Path "$PSScriptRoot/../../../source/Public" -Filter "Get-LPMSAppRoleAssignment.ps1" -ErrorAction SilentlyContinue
        ...
        if ($publicFunction) {
            ...
            throw "Could not find Get-LPMSAppRoleAssignment.ps1"
        }
    }
}

Describe 'Get-LPMSAppRoleAssignment' {
    Context 'Parameter Validation' {
        It 'Should have no mandatory parameters' {
            $command = Get-Command -Name Get-LPMSAppRoleAssignment
